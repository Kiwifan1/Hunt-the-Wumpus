...
public class GameControl {
	...
	// following e-nums are for the start of the game and anywhere else so its
	...
	public static String startGame(GraphicalInterface GI, Player player, Cave cave, Trivia triv) {
		...
		// the while loop that runs the game while the player is alive, has arrows, and
		// has coins
		...
		while (player.getArrows() > 0 && player.getCoins() > 0 || player.getTurns() == 0) {
			...
			response = GI.getRoom(rooms[0], rooms[1], rooms[2], hazards, player.getTurns(), player.getCoins(),
					player.getArrows());
			// if player moves
			if (response > 0) {
				player.movePlayer();
				...
				Sounds.movePlayer();
				room = GameLocations.getPlayerLocation();
				GraphicalInterface.betweenTurns(triv.giveTrivia(), room, player.getTurns(), player.getCoins(),
						player.getArrows(), false, 0);
				...
				if (response == ARROW) {
					...
					arrowShot = GI.shootArrow(rooms[0], rooms[1], rooms[2], hazards, player.getTurns(),
							player.getCoins(), player.getArrows());
					...
					if (arrowShot != 0) {
						...
						if (GameLocations.shootArrow(arrowShot)) {
							...
							Sounds.moveWumpus();
							GraphicalInterface.arrowHit(false, player.getArrows());// add code for GI that states you
																					// missed
						}
					}
				...
				// if player wants to leave game but not exit
					...
					int caveSelect = GI.mainmenu(HighScore.getCaves(), HighScore.getNames(), HighScore.getScores(),
							true);
					if (caveSelect > 0) {
	 ...
	 * 
	 */
	public static void endGame(int caveName, GraphicalInterface GI, String Name, Player player, String reason,
			int score) {
	...
	public static boolean trivia(Trivia triv, GraphicalInterface GI, Player player, int numQ, int numC) {
		...
		int count = 0;
		int wrong = 0;
		// while the player has answered less than required questions or required
		// correct questions
		while (correct < numC && count < numQ && wrong < numC) {
			char answer = GraphicalInterface.getAnswer(triv.getQuestion(), triv.getA(), triv.getB(), triv.getC(),
					triv.getD());
